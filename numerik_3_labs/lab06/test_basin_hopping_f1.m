...
% function handle
f1 = @(x) x(1).^2+x(2).^2+10*(2-cos(2*pi*x(1))-cos(2*pi*x(2)));
...
loc_opt = @(x) hooke_jeeves(f1, x, h_loc, tol); 

% perform basin hopping
X = basin_hopping(f1, x0, T, loc_opt, h_bas, num_it);

% plot contour lines
figure('Position', [100,100,1000,700]);
[x,y] = meshgrid(min(X(1,:))-1:0.01:max(X(1,:))+1,min(X(2,:))-1:0.01:max(X(2,:))+1);
f = @(x,y) x.^2+y.^2+10*(2-cos(2*pi*x)-cos(2*pi*y));
z = f(x,y);
