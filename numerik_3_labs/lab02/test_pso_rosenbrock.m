...
% define function
F = @(x,y) [ 1-x; 10*(y-x.^2); sqrt(x.^2+y.^2) ];
f = @(x,y) F(x,y)'*F(x,y);
...
c_p = 1.5; % weight particle's best 1.5
c_g = 1.5; % weight global best 1.5
inertia = 0.7;
max_it = 300; % max number iteration steps
max_noupd = 20; % max number successive iteration steps without new global best

% plot initial setting
figure('Position', [100,100,1000,700]);
[x,y] = meshgrid(-pmax:0.05:pmax,-pmax:0.05:pmax);
z = (1-x).^2 + 100*(y-x.^2).^2 + x.^2+y.^2;
contour(x,y,z,1000);
shading interp;
colormap(turbo(256));
hold on;
...
plot(exact(1),exact(2),'x','Color',[1 0.5 0],'MarkerSize',20,'LineWidth',2);
...
legend('', 'Particles', 'Velocities', 'unique global minimum', 'current global best', 'Interpreter', 'latex', 'FontSize',12);
pause(5);

% perform particle swarm optimization
particle_swarm_optimization(f, p0, v0, inertia, c_p, c_g, max_it, max_noupd,p_plot,v_plot,g_plot,t_plot,vmax);